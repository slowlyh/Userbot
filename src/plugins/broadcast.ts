...
function loadWhitelist(): WLConfig {
  ...
  if (envWL) {
    cfg.enabled = true;
    cfg.allow.push(...envWL.split(',').map(s => s.trim()).filter(Boolean));
  }
  return cfg;
}
...
const isGroup   = (e: any, d: any) =>
  d?.isGroup || (e?.className === 'Channel' && e?.megagroup) || e?.className === 'Chat';
...
async function floodAwareSendMsg(client: any, peer: any, payload: any) {
  while (true) {
    try {
      return await client.sendMessage(peer, payload);
    } catch (e: any) {
...
async function floodAwareSendFile(client: any, peer: any, payload: any) {
  while (true) {
    try {
      return await client.sendFile(peer, payload);
    } catch (e: any) {
      const msg = String(e?.message || e);
      const m = msg.match(/FLOOD_WAIT_(\d+)/) || msg.match(/wait of (\d+) seconds/i);
      const seconds = (e?.seconds as number) || (m ? Number(m[1]) : 0);
...
const plugin: PluginModule = {
  ...
  handler: async ({ event, text }, ctx) => {
    ...
    if (!isOwner(event, meId)) {
      await client.sendMessage(peer, { message: 'Unauthorized.' });
      return;
    }
    ...
    // Flags: --only=groups|channels, --md, --html, --limit=N, --except=a,b,c
    ...
    let mediaBuf: Buffer | string | null = null; // ← perbaiki tipe di sini
    if (replyId) {
      try {
        const msgs = await client.getMessages(peer, { ids: [replyId] });
        const r = Array.isArray(msgs) ? msgs[0] : msgs;
        if (r && (r as any).media) {
          const dl = await client.downloadMedia(r);  // string | Buffer | undefined
          if (dl !== undefined && dl !== null) mediaBuf = dl as Buffer | string;
        }
      } catch {}
    }

    // ← ganti baris Python-like dengan TS yang benar
    if (!content.trim() && !mediaBuf) {
      await client.sendMessage(peer, { message: 'Usage: .broadcast [--only=groups|channels] [--md|--html] [--limit=N] [--except=a,b] <message multiline>\nOr reply to a media and use `.broadcast <caption>`' });
      return;
    }
    ...
    for (const p of targets) {
      try {
        if (mediaBuf) {
          await floodAwareSendFile(client, p as any, {
            file: mediaBuf as any,          // string atau Buffer keduanya valid
